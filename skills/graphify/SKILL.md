...
- **graphify IS for documents, not just code.** Despite the `update` command being AST-focused, the full pipeline (`/graphify <path>`) handles markdown, PDFs, and plain text through semantic extraction. The YouTube demos showing 71.5x token savings use graphify on document corpora, not codebases.
- **`graphify add` only accepts URLs (http/https).** Local files cannot be ingested via `add`. To add local markdown/wiki pages to the graph, run `/graphify <path>` or `/graphify <path> --update` on the entire directory.
- **Edge flow animations are query-time, not constant.** Some demos show white dots tracing paths — these represent agent exploration during `query`/`path` commands, not idle decoration. Do NOT add constant-running edge animations to the HTML output; they cause lag and serve no purpose.
- **Obsidian Vault import pattern:** Raw vault → `wiki/raw/` (preserve) → llm-wiki ingests → entities/ + concepts/ pages → `/graphify wiki/ --update` to rebuild graph. Frontmatter-based extraction can auto-generate nodes/edges from `[[wikilinks]]`.
- **Graphify is not a live search index.** When the user says they added something to the wiki and graphify, do not assume `graphify-out/graph.json` actually contains the new page. First verify with `grep`/JSON search against both `graphify-out/manifest.json` and `graphify-out/graph.json` for the page slug/key terms. If absent, say explicitly: “wiki has it, current graph does not.” Then search the wiki source (`index.md`, `CLAUDE.md`, and relevant `entities/` pages) directly instead of doing broad graph traversals. A `GRAPH_REPORT.md` showing “cluster-only mode” and `Token cost: 0` is a strong signal that markdown semantic extraction did not run.
- **DO NOT dispatch worker batches before confirming the worker invocation path actually works.** Run a single small smoke test against ONE batch first, parse the output, confirm JSON returns. Only then fan out to 3+. Failure modes that bit a previous run, in order: (1) `hermes chat` injects ~50K tokens of system prompt + tools + memory, blowing the Sonnet OAuth long-context tier and getting HTTP 429 "Extra usage required for long context requests"; (2) direct Anthropic API call without `hermes_claude_auth` bypass returns HTTP 401 "Invalid authentication credentials" — bypass is a Python import hook tied to a specific venv, not a global; (3) parsers that scan for the first `{...}` block can grab the JSON SCHEMA EXAMPLE in the prompt instead of the model's actual response. See `references/document-extraction-recipe.md` for the working invocation.
- **For document/markdown semantic extraction with Sonnet OAuth, use `hermes chat` with all context-injection switched off**: `--ignore-user-config --ignore-rules -t '' --max-turns 1`. This is what makes the request small enough to fit under the standard tier and avoid 429. The bare CLI without these flags will fail.
- **JSON parsing of model output**: never grab the first `{...}` from raw text. The prompt itself contains the JSON SCHEMA as a literal example block. Search for the LAST `{...}` block, or instruct the model to wrap output in a unique sentinel (`<<<JSON>>>...<<</JSON>>>`) and extract between sentinels.
...
- **`graphify add` only accepts http/https URLs** — local file paths fail with "Blocked URL scheme". Use `graphify update <dir>` for local files.
- **CLI `graphify update <wiki>` does NOT semantically refresh markdown.** It only re-runs AST on changed code files; if a corpus is markdown-only it prints "No code files found - nothing to rebuild." To refresh markdown nodes/edges you must invoke the slash command `/graphify <path> --update` from inside an LLM-capable agent (Claude Code, Hermes chat, etc.) so semantic extraction subagents fire. Bare CLI is for code-only fast paths.
- **`hermes chat -q ...` hits HTTP 429 "Extra usage required for long context" on Sonnet/Opus OAuth even for small prompts.** Hermes injects ~50K of tool descriptions, memory, and skill listings into every chat invocation. A 35K-char extraction prompt becomes a 1M+ token request that the Max OAuth tier rejects. **DO NOT auto-fall-back to direct Anthropic API calls without user consent** — that path bills the user for pay-per-token usage and explicitly violates the operator's `Provider Path Discipline` rule (see `responding-to-user/references/provider-path-discipline.md`). Correct response sequence: (1) stop, (2) report the 429 to the user, (3) present options — smaller batches with `--ignore-user-config --ignore-rules -t '' --max-turns 1`, switch to a smaller model, switch provider with explicit consent, abort, (4) wait for user choice. The direct-API recipe in `references/anthropic-direct-api-extraction.md` and `scripts/extract.py` exist as a *user-approved* tool, not as an autonomous fallback.
- **graphify-out/ is large** — add to .gitignore. Auto-generated binaries (graph.html, graph.json).
- **Don't add constant animations.** YouTube "white dots flowing" is D3.js custom, not Graphify default. Users want query-path animations, not all-edge animations.
- **Small graphs (< 50 nodes) look sparse** — normal. Grows with knowledge. Don't compensate with artificial effects.
- **graph.html fails for > 5,000 nodes** — use Obsidian vault or graph.json instead.
- **`file_type` mismatch with wiki SCHEMA.** Graphify's `build_from_json` only accepts `code | concept | document | image | paper | rationale`. Wiki SCHEMA.md additionally allows `entity | project | system | character | architecture | comparison | query | summary` — these all fall through to a warning per node ("Node X has invalid file_type 'Y' - must be one of [...]"). The graph still builds correctly; the warning is benign. If you see a flood of these (89 warnings on a 2026-05-07 wiki run), that's expected, not a bug. Map them to `document` at ingest if you want a clean log; do NOT change the wiki SCHEMA — it's the SoT.

## Support Files

- `scripts/extract.py` — minimal Anthropic API caller for semantic extraction. Reads a prompt file, returns parsed nodes/edges JSON. **Only use after explicit user approval — this routes through pay-per-token, not Max subscription.** Default path is always `hermes chat`.
- `references/anthropic-direct-api-extraction.md` — why `hermes chat` fails for graphify subagents on Max OAuth, the exact 429 error, the bypass headers/system prompt that work, AND the rule that this path requires user consent before invocation.
- `references/document-extraction-recipe.md` — `hermes chat` subprocess path. Use when running from cron, CI, or any context without `delegate_task`. Smoke-test-one-batch-first protocol, sentinel-wrapped JSON pattern, fallback model order.
- `references/incremental-wiki-update-via-delegate-task.md` — `delegate_task` in-process path. Use when the agent IS the orchestrator. Detect → chunk → dispatch ≤3 workers with `toolsets: ["file"]` → merge in-process → build/cluster/HTML → `save_manifest`. Includes 2026-05-07 cost benchmarks (33 files / 3 workers / ~3 min / 493K input tokens vs 2 files / 1 worker / 31 sec / 44K).
